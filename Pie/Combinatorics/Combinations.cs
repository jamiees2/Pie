...
namespace Pie.Combinatorics
{
    ...
    internal class Combinations<T> : IMetaCollection<T>
    {
        #region Constructors
        ...
        public class Enumerator : IEnumerator<IList<T>>
        {
            ...
            private void ComputeCurrent()
            {
                if (myCurrentList == null)
                {
                    ...
                    foreach (bool t in currentPermutation)
                    {
                        if (t == false)
                        {
        ...
        private void Initialize(IList<T> values, int lowerIndex, GenerateOption type)
        {
            ...
            {
                myMap.AddRange(myValues.Select((t, i) => i < myValues.Count - myLowerIndex));
            }
            else
            {
